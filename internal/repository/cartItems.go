...
func (ci *CartItemsRepository) GetCartItemsByID(tx *gorm.DB, cartItemsID int) (*entity.CartItems, error) {
	var cartItems *entity.CartItems
	err := tx.Debug().Where("cart_items_id = ?", cartItemsID).First(&cartItems).Error
	if err != nil {
		return nil, err
	}
